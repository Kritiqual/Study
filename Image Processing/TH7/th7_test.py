...
folder = "/home/kritiqual/Projects/Study/Image Processing/Input/TH7/"
train_filenames = [
    "train1.bmp",
    "train2.bmp",
    "train3.bmp",
...
plt.subplot(2, 2, 1)
plt.imshow(
    cv2.cvtColor(cv2.imread(folder + test_filenames[test_index]), cv2.COLOR_BGR2RGB)
)
...
for i in range(k):
    ...
    plt.imshow(
        cv2.cvtColor(
            cv2.imread(folder + train_filenames[indices[0][i]]), cv2.COLOR_BGR2RGB
        )
    )
...
plt.subplot(2, 2, 1)
plt.imshow(
    cv2.cvtColor(cv2.imread(folder_test + test_filenames[test_index]), cv2.COLOR_BGR2RGB)
)
plt.title("Test image")
