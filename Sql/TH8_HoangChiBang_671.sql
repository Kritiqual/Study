...
--26,
SELECT SUM(SLCuoi) AS N'Tổng tồn kho theo năm'
FROM TonKho
GROUP BY YEAR(NamThang)
--<~>--
SELECT SUM(SLCuoi) AS N'Tổng tồn kho theo tháng'
FROM TonKho
GROUP BY RIGHT(NamThang, 2)
-- 27,
SELECT dh.SoDH, COUNT(ct.SoDH) AS SLPNhap
FROM DonDH dh INNER JOIN CTDonDH ct
    ON dh.SoDH = ct.SoDH
GROUP BY dh.SoDH
-- 28,
SELECT COUNT(SoDH) AS SLDonDH
FROM DonDH
WHERE MONTH(NgayDH)=1 AND YEAR(NgayDH)=2002
-- 29,
SELECT SoDH
FROM PNhap pn INNER JOIN CTPNhap ct
ON pn.SoPN = ct.SoPN
GROUP BY SoDH
HAVING COUNT(MaVT) < 5
-- 30,
